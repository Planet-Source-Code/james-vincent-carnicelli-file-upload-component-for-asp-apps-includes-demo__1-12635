...
'Determine whether or not the given array begins with the items
'in the test array.  This only works if both arrays contain
'intrinsically comparable data types.
Public Function ArrayBeginsWith(List, TestList) As Boolean
    Dim i
    If UBound(List) < UBound(TestList) Then Exit Function
    For i = 0 To UBound(TestList)
        If List(i) <> TestList(i) Then Exit Function
    Next
    ArrayBeginsWith = True
End Function


'Returns the index of the first (or next) item with the specified
'value or -1 if not found.
Public Function ArrayFind(List, Value, Optional StartAt As Integer = 0)
    Dim i As Integer
    For i = StartAt To UBound(List)
        If IsObject(Value) Then
            If IsObject(List) Then
                'No-brainer object reference comparison
                If List(i) Is Value Then
                    ArrayFind = i
                    Exit Function
                End If
            End If
        Else
            If Not IsObject(List(i)) Then
                'Hopefully the data types are comparable
