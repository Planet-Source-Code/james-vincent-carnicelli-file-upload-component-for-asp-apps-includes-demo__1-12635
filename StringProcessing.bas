...
'Transform the byte array into a plain-text string
Public Function ByteArrayToText(ByteArray) As String
    Dim i As Integer
    ByteArrayToText = String(UBound(ByteArray) + 1, " ")
    For i = 0 To UBound(ByteArray)
        Mid(ByteArrayToText, i + 1, 1) = Chr(ByteArray(i))
    Next
End Function

'Transform the byte array into a plain-text string
Public Function TextToByteArray(Text As String)
    Dim i As Integer, ByteArray
    If Text = "" Then
        ByteArray = Array()
    End If
    ReDim ByteArray(Len(Text) - 1) As Byte
    For i = 1 To Len(Text)
        ByteArray(i - 1) = CByte(Asc(Mid(Text, i, 1)))
    Next
    TextToByteArray = ByteArray
End Function
